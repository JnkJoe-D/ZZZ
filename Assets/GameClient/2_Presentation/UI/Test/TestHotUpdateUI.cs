...
namespace Game.UI
{
    ...
    public class TestHotUpdateUI : MonoBehaviour
    {
        private IEnumerator Start()
        {
            ...
            GLog.Info(LogTags.UI, "<color=green>[Test] 游戏框架启动完毕，开始测试热更 UI！</color>");
            
            // 稍微等待一下表现效果
            yield return new WaitForSeconds(1.5f);

            // 3. 模拟底层 ResourceUpdater 发现更新并触发拦截弹窗
            GLog.Info(LogTags.UI, "<color=yellow>[Test] 模拟：发现新版本，弹出确认框...</color>");
            ...
            EventCenter.Publish(new HotUpdateRequireConfirmEvent
            {
                ...
                ConfirmAction = () => 
                {
                    GLog.Info(LogTags.UI, "<color=green>[Test] 用户点击了确认，开始模拟下载！</color>");
                    isConfirmed = true;
                }
            });
            ...
            GLog.Info(LogTags.UI, "<color=green>[Test] 模拟下载完成！触发资源就绪事件！</color>");
