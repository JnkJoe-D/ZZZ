...
namespace Game.UI
{
    ...
    public class HotUpdateModule : UIModule<HotUpdateView, HotUpdateModel>
    {
        ...
        private void OnUpdateCompleted(HotUpdateCompletedEvent e)
        {
            ...
            // 所以先不开始网络连动。仅仅报告状态
            GLog.Info(LogTags.UI, "资源就绪，等待整个引擎初始化结束...");
        ...
        {
            GLog.Info(LogTags.UI, "核心就绪，转入连接服务器动画阶段...");
        ...
        private void OnUpdateFailed(HotUpdateFailedEvent e)
        {
            ...
            switch (e.Reason)
            {
                case Game.Framework.HotUpdateFailReason.InitializeFailed:
                    statusMsg = "初始化资源系统失败";
                    break;
                case Game.Framework.HotUpdateFailReason.VersionRequestFailed:
                    statusMsg = "无法连接到资源服务器";
                    break;
                case Game.Framework.HotUpdateFailReason.ManifestUpdateFailed:
                    statusMsg = "检查资源更新失败";
                    break;
                case Game.Framework.HotUpdateFailReason.DownloadFailed:
                    statusMsg = "下载资源文件失败";
                    break;
                default:
            ...
            UIManager.Instance.Open<MessageBoxModule>(new MessageBoxModel
            {
                ...
                OnConfirm = () => 
                {
                    ...
                    View?.UpdateView(Model);
                    EventCenter.Publish(new Game.Framework.HotUpdateRetryEvent());
                },
                OnCancel = () => 
                {
#if UNITY_EDITOR
