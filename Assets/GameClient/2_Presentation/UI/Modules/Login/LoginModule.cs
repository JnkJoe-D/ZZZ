...
namespace Game.UI
{
    ...
    public class LoginModule : UIModule<LoginView, LoginModel>
    {
        ...
        private void OnLoginClick()
        {
            ...
            GLog.Info(LogTags.UI, $"请求登录 Account: {account} （由于安全原因隐去了密码打印）");
        ...
        private void OnLoginResponse(S2C_Login response)
        {
            ...
            GLog.Info(LogTags.UI, $"收到登录响应 Code: {response.Code}, Message: {response.Message}");
            ...
            if (response.Code == (int)ErrorCode.Success)
            {
                ...
                // 假设目标场景名为 "MainLobby"
                SceneManager.Instance.ChangeScene(new SceneTransitionParams
                {
        ...
        {
            GLog.Info(LogTags.UI, "点击注册，打开独立注册面板...");
