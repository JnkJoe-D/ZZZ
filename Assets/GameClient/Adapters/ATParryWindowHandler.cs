                ...
                foreach (var effect in hitEffectCfg.Effects)
                {
                    switch (effect.EffectType)
                    {
                        case cfg.ZZZ.HitEffectType.ModifyAttribute:
                            ...
                            {
                                ctx.Attacker.AttributeResolver.ModifyAttribute((Game.Logic.AttributeId)effect.AttrId, effect.Value);
                            }
                            else if (ctx.Attacker.StatusModule?.Attributes != null)
                            {
                                var targetAttrId = (Game.Logic.AttributeId)effect.AttrId;
                            ...
                            Debug.Log($"<color=cyan>[Combat][ParryExecute] 招架反制生效通用属性修改: AttrId={effect.AttrId}, Value={effect.Value} -> 目标: {ctx.Attacker.name}</color>");
                            break;
