...
namespace ATEditor.Editor
{
    public class ClipDrawer : SkillInspectorBase
    {
        ...
        public virtual void DrawTimelineGUI(ClipBase clip, Rect clipRect, ATEditorState state, Color clipColor, string displayName)
        {
            if (!clip.isEnabled) clipColor.a *= 0.5f;

            EditorGUI.DrawRect(clipRect, clipColor);
            if (!clip.isEnabled) EditorGUI.DrawRect(clipRect, ATEditorStyles.ClipDisabledOverlayColor);

            EditorGUI.DrawRect(new Rect(clipRect.x, clipRect.y, clipRect.width, 1), ATEditorStyles.ClipDefaultBorderColor);
            EditorGUI.DrawRect(new Rect(clipRect.x, clipRect.y + clipRect.height - 1, clipRect.width, 1), ATEditorStyles.ClipDefaultBorderColor);
            EditorGUI.DrawRect(new Rect(clipRect.x, clipRect.y, 1, clipRect.height), ATEditorStyles.ClipDefaultBorderColor);
            EditorGUI.DrawRect(new Rect(clipRect.x + clipRect.width - 1, clipRect.y, 1, clipRect.height), ATEditorStyles.ClipDefaultBorderColor);

            if (state.selectedClips.Contains(clip))
            {
                Color highlightColor = ATEditorStyles.ClipSelectedBorderColor;
            ...
            if (clip.SupportsBlending)
            {
                if (clip.BlendInDuration > 0)
                {
                    ...
                    Handles.BeginGUI();
                    Handles.color = ATEditorStyles.BlendAreaFillColor;
                    Handles.DrawAAConvexPolygon(verts);

                    Handles.color = ATEditorStyles.BlendAreaLineColor;
