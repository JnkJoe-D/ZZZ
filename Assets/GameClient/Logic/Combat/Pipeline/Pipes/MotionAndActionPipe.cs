    ...
    public class MotionAndActionPipe : IHitPipe
    {
        ...
        public void Process(HitPipelineContext ctx)
        {
            ...
            if (ctx.SelectedReactionType != HitReactionType.None)
            {
                ...
                Debug.Log($"<color=orange>[HitPipeline] 受击表现: {ctx.Victim.name} | 类型: {ctx.SelectedReactionType} | 动作: {resolvedAction?.name ?? "None"} | 转向: {needFaceAttacker} (角:{signedHorizontalAngle:F1}°) | 攻击来源: {ctx.Attacker?.name}</color>");
