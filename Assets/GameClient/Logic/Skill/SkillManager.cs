...
namespace Game.Logic.Skill
{
    /// <summary>
    /// 全局静态技能管理器（非Mono），用于复用 Timeline 数据和角色的播放器
    /// </summary>
    public class SkillManager : Game.Framework.Singleton<SkillManager>
    {
        ...
        public SkillTimeline GetOrLoadTimeline(SkillConfigSO config)
        {
            if (config == null || config.TimelineAsset == null) return null;
            
            if (_timelineCache.TryGetValue(config.SkillID, out var timeline))
                return timeline;

            timeline = SerializationUtility.OpenFromJson(config.TimelineAsset);
            if (timeline != null) _timelineCache[config.SkillID] = timeline;
