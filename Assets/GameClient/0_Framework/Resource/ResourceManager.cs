...
namespace Game.Framework
{
    ...
    public class ResourceManager : Game.Framework.Singleton<ResourceManager>
    {
        ...
        public IEnumerator InitializeAsync(ResourceConfig config, MonoBehaviour runner)
        {
            ...
            GLog.Info(LogTags.Resource, $"初始化，模式: {config.playMode}，包名: {config.defaultPackageName}");
            ...
            switch (config.playMode)
            {
                ...
                default:
                    GLog.Error(LogTags.Resource, $"不支持的运行模式: {config.playMode}");
                    yield break;
            }
            ...
            {
                GLog.Error(LogTags.Resource, $"Package 初始化失败: {initOp.Error}");
            ...
            GLog.Info(LogTags.Resource, "Package 初始化成功");
            ...
            else
            {
                ...
                GLog.Info(LogTags.Resource, $"当前模式 {config.playMode}，正在加载本地清单...");
                
                var versionOp = _package.RequestPackageVersionAsync(appendTimeTicks: false);
                yield return versionOp;
                if (versionOp.Status != EOperationStatus.Succeed)
                {
                    GLog.Error(LogTags.Resource, $"获取本地版本失败: {versionOp.Error}");
                    yield break;
                }
                ...
                {
                    GLog.Error(LogTags.Resource, $"激活本地清单失败: {manifestOp.Error}");
                    yield break;
                }
                
                GLog.Info(LogTags.Resource, $"本地清单加载成功，版本号: {versionOp.PackageVersion}");
            }

            // ── 5. 完成 ──────────────────────────────────────
            IsInitialized = true;
            EventCenter.Publish(new ResourceInitializedEvent());
            GLog.Info(LogTags.Resource, "初始化完成，资源系统就绪");
        }
...
#else
            // 非编辑器环境降级为离线模式
            GLog.Warning(LogTags.Resource, "EditorSimulate 模式仅支持编辑器，已自动降级为 Offline 模式");
            return InitOffline();
#endif
        }
        ...
        public void SetRemoteVersion(string version)
        {
            if (_remoteServices != null)
            {
                ...
                GLog.Info(LogTags.Resource, $"远程服务版本已更新: {version}");
            }
        }
        ...
        public T LoadAsset<T>(string assetPath) where T : UnityEngine.Object
        {
            ...
            {
                GLog.Error(LogTags.Resource, $"同步加载失败: {assetPath}");
        ...
        public async Task<T> LoadSubAssetAsync<T>(string assetPath, string subAssetName) where T : UnityEngine.Object
        {
            ...
            if (handle.Status != EOperationStatus.Succeed)
            {
                GLog.Error(LogTags.Resource, $"异步加载子资源失败: {assetPath}[{subAssetName}] | {handle.LastError}");
                EventCenter.Publish(new AssetLoadFailedEvent { AssetPath = assetPath, Error = handle.LastError });
                return null;
            }
            ...
            if (subAsset == null)
            {
                GLog.Error(LogTags.Resource, $"在 {assetPath} 中找不到子资源: {subAssetName}");
            }
            return subAsset;
        }
        ...
        public T LoadSubAsset<T>(string assetPath, string subAssetName) where T : UnityEngine.Object
        {
            ...
            if (handle.Status != EOperationStatus.Succeed)
            {
                GLog.Error(LogTags.Resource, $"同步加载子资源失败: {assetPath}[{subAssetName}] | {handle.LastError}");
                return null;
            }
        ...
        ) where T : UnityEngine.Object
        {
            ...
            if (handle.Status != EOperationStatus.Succeed)
            {
                GLog.Error(LogTags.Resource, $"异步加载失败: {assetPath} | {handle.LastError}");
                EventCenter.Publish(new AssetLoadFailedEvent { AssetPath = assetPath, Error = handle.LastError });
        ...
        public async Task<T> LoadAssetAsync<T>(string assetPath, Action<float> onProgress = null) where T : UnityEngine.Object
        {
            ...
            if (handle.Status != EOperationStatus.Succeed)
            {
                GLog.Error(LogTags.Resource, $"异步加载(Task)失败: {assetPath} | {handle.LastError}");
                EventCenter.Publish(new AssetLoadFailedEvent { AssetPath = assetPath, Error = handle.LastError });
                return null;
            }
        ...
        )
        {
            ...
            if (handle.Status != EOperationStatus.Succeed)
            {
                GLog.Error(LogTags.Resource, $"实例化失败: {assetPath} | {handle.LastError}");
                EventCenter.Publish(new AssetLoadFailedEvent { AssetPath = assetPath, Error = handle.LastError });
                onComplete?.Invoke(null);
                yield break;
            }
        ...
        )
        {
            ...
            if (handle.Status != EOperationStatus.Succeed)
            {
                GLog.Error(LogTags.Resource, $"场景加载失败: {scenePath} | {handle.LastError}");
        ...
        public void Shutdown()
        {
            ...
            ReleaseAllAssets();
            YooAssets.Destroy();
            GLog.Info(LogTags.Resource, "已关闭");
        }
