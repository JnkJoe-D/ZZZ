...
namespace Game.Framework
{
    ...
    public class TcpChannel : IDisposable
    {
        ...
        public void Connect(string host, int port)
        {
            if (IsConnected)
            {
                GLog.Warning(LogTags.Tcp, "已连接，请先断开");
                return;
            }
            ...
            try
            {
                ...
                GLog.Info(LogTags.Tcp, $"已连接: {host}:{port}");
            }
            catch (Exception e)
            {
                GLog.Warning(LogTags.Tcp, $"连接请求被拒绝: {e.Message}");
                Cleanup();
        ...
        public void Send(ushort msgId, byte[] payload)
        {
            if (!IsConnected)
            {
                GLog.Warning(LogTags.Tcp, $"未连接，无法发送 0x{msgId:X4}");
                return;
            }
            ...
            lock (_sendLock)
            {
                ...
                {
                    GLog.Error(LogTags.Tcp, $"发送失败: {e.Message}");
        ...
        private void ReceiveLoop()
        {
            try
            {
                while (_isRunning)
                {
                    ...
                    if (available <= 0)
                    {
                        GLog.Error(LogTags.Tcp, "接收缓冲区溢出");
                        break;
                    }
        ...
        private void HandleDisconnect(string reason)
        {
            ...
            _isRunning = false;
            GLog.Info(LogTags.Tcp, $"断开: {reason}");
