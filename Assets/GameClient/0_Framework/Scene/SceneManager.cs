...
namespace Game.Framework
{
    ...
    public class SceneManager : Game.Framework.Singleton<SceneManager>
    {
        ...
        public void ChangeScene(SceneTransitionParams transitionParams)
        {
            if (IsLoading)
            {
                GLog.Warning(LogTags.Scene, $"当前正在加载场景中，忽略请求: {transitionParams.SceneName}");
                return;
            }

            if (transitionParams == null || string.IsNullOrEmpty(transitionParams.SceneName))
            {
                GLog.Error(LogTags.Scene, "转场参数为空或场景名称无效！");
                return;
            }
        ...
        {
            IsLoading = true;
            GLog.Info(LogTags.Scene, $"开始切换场景: {CurrentSceneName} -> {p.SceneName}");
            ...
            GLog.Info(LogTags.Scene, $"场景及关联资产切换完成: {p.SceneName}");
        }
        ...
        {
            GLog.Info(LogTags.Scene, $"开始叠加加载场景: {sceneName}");
            ...
            if (isLoadSuccess)
            {
                GLog.Info(LogTags.Scene, $"叠加场景加载成功: {sceneName}");
            }
        }
