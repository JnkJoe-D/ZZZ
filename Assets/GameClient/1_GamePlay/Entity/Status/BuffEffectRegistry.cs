...
namespace Game.GamePlay
{
    ...
    public static class BuffEffectRegistry
    {
        ...
        public static void Initialize()
        {
            ...
            foreach (var assembly in assemblies)
            {
                ...
                foreach (var type in types)
                {
                    ...
                    {
                        GLog.Error(LogTags.Buff, $"执行器 {type.Name} 缺少标准构造函数 (BuffEffectData[, BuffApplyContext])！");
                    }
                }
            }
        ...
        public static IBuffEffect Create(BuffEffectData data, BuffApplyContext ctx)
        {
            ...
            GLog.Warning(LogTags.Buff, $"未找到效果类型为 {data.EffectType} 的 BuffEffect 执行器绑定。请确认对应执行器已添加 [BuffEffectBinding] 特性。");
            return null;
        }
