...
public class Test_Switch : MonoBehaviour
{
    ...
    public void Start()
    {
        ...
        {
            GLog.Error(LogTags.Team, $"测试初始化失败: {ex.Message}");
        }
    }
    void Update()
    {
        try
        {
            if(Input.GetKeyDown(KeyCode.Alpha1))
            {
                ...
                GLog.Info(LogTags.Team, $"播放开始: timeline1");
            }
            if (Input.GetKeyDown(KeyCode.Alpha2))
            {
                runner.Play(timeline2, context);
                GLog.Info(LogTags.Team, $"播放开始: timeline2");
            }
            if (runner != null)
            {
        ...
        {
            GLog.Error(LogTags.Team, $"测试运行时异常: {ex.Message}");
        }
    }
}
