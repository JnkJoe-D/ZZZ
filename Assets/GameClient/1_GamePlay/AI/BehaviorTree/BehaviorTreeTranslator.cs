    ...
    public static class BehaviorTreeTranslator
    {
        ...
        private static Node TranslateNodeInternal(NodeData data, Blackboard bb, TreeActionAgent agent, Dictionary<string, Node> map)
        {
            ...
            switch (data)
            {
                ...
                case MonsterCoolingStrafeData strafeData:
                    if (agent != null) {
                        return new MonsterCoolingStrafeTask(
                            strafeData,
                            agent
                        );
                    }
                ...
                case MultiFrameDebugData multiDebugData:
                    return new MultiFrameDebugTask(
                ...
                case DebugData debug:
                    return new NPBehave.Action(() => { GLog.Info(LogTags.BehaviorTree, debug.message); });
                ...
                default:
                    GLog.Warning(LogTags.BehaviorTree, $"Unsupported NodeData type: {data.GetType().Name}");
                    return null;
            }
        }
