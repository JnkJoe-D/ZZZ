...
namespace Game.GamePlay
{
    ...
    public class PlayerManager : Game.Framework.Singleton<PlayerManager>
    {
        ...
        {
            GLog.Info(LogTags.Player, "Initialized.");
        }
        ...
        public async Task<RoleEntity> PossessNewCharacterAsync(string characterPrefabPath, CharacterConfigAsset config, Vector3 spawnPos, Quaternion spawnRot)
        {
            ...
            {
                GLog.Error(LogTags.Player, $"Failed to load character prefab: {characterPrefabPath}");
                return null;
            }

            // 2. 实例化躯壳
            if (Game.GamePlay.ActionManager.Instance != null)
            {
                await Game.GamePlay.ActionManager.Instance.PreloadCharacterActionsAsync(config);
            }
            ...
            GLog.Info(LogTags.Player, $"Successfully possessed new character: {config.Name}");
        ...
        public void UnpossessCurrentCharacter()
        {
            if (LocalCharacter != null)
            {
                ...
                GLog.Info(LogTags.Player, "Unpossessed current character.");
            }
        }
