    ...
    public class AnimState : StateBase
    {
        ...
        protected override Playable CreatePlayable(PlayableGraph graph)
        {
            ...
            catch (MissingReferenceException)
            {
                ...
                MAnimLog.Warning("Skipped creating playable because the AnimationClip was already destroyed or unloaded.");
        ...
        public void BindClip(AnimationClip clip, PlayableGraph graph)
        {
            ...
            if (!UpdateClipMetadata(clip))
            {
                MAnimLog.Warning("Skipped binding an AnimationClip because it was already destroyed or unloaded.");
                return;
            }
            ...
            if (!_playableCache.IsValid() || clipChanged)
            {
                ...
                catch (MissingReferenceException)
                {
                    ...
                    _cachedLength = 0f;
                    _cachedIsLooping = false;
                    MAnimLog.Warning("Skipped rebuilding playable because the AnimationClip was destroyed or unloaded during bind.");
                    return;
                }
