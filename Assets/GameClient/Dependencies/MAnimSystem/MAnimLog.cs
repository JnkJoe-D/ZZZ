...
{
    /// <summary>
    /// MAnimSystem 动画图系统内部独立日志门面。
    /// 完全自包含，不依赖任何外部业务框架（对 Game.Framework 零耦合）。
    /// 默认回退至 UnityEngine.Debug，支持宿主工程在启动时注入委托插槽桥接统一日志体系。
    /// </summary>
    public static class MAnimLog
    {
        // 外部可注入的委托插槽
        public static Action<string> LogHandler     = msg => UnityEngine.Debug.Log($"[MAnimSystem] {msg}");
        public static Action<string> WarningHandler = msg => UnityEngine.Debug.LogWarning($"[MAnimSystem] {msg}");
        public static Action<string> ErrorHandler   = msg => UnityEngine.Debug.LogError($"[MAnimSystem] {msg}");
