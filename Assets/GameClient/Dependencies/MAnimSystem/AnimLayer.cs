...
namespace MAnimSystem
{
    ...
    public class AnimLayer
    {
        ...
        private bool PrepareStateForPlay(StateBase state)
        {
            ...
            {
                MAnimLog.Error($"State owner mismatch. state={state.GetType().Name}, ownerLayer={state.ParentLayer.LayerIndex}, playLayer={LayerIndex}");
                return false;
            }

            if (!state.EnsureInitialized(this, Graph))
            {
                MAnimLog.Error($"Failed to initialize state. state={state.GetType().Name}, layer={LayerIndex}");
                return false;
            }

            if (!state.Playable.IsValid())
            {
                MAnimLog.Error($"State playable is invalid. state={state.GetType().Name}, layer={LayerIndex}");
                return false;
            }
