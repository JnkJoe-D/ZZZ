...
public class Test1 : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (UnityEngine.Input.GetKeyDown(KeyCode.E))
        {
            ...
            // 订阅过渡完成事件 (淡入完毕)
            state.OnFadeComplete = (_) => MAnimLog.Info("[事件] 攻击动作：过渡淡入完成!");
            
            // 订阅播放结束事件 (动画播完)
            state.OnEnd = (_) => 
            {
                MAnimLog.Info("[事件] 攻击动作：播放结束! 自动切回 Idle。");
                animComponent.Play(idleClip, 0.2f);
            };
        }
    ...
    IEnumerator RapidSwitchTest()
    {
        ...
        MAnimLog.Info("[频繁切换测试] 开始 - 验证中断列表法");
        ...
        for (int i = 0; i < switchCount; i++)
        {
            ...
            MAnimLog.Info($"[频繁切换] 第 {i + 1} 次切换: {clip.name}");
            
            // 50ms 切换一次，远小于过渡时间 250ms
            yield return new WaitForSeconds(0.05f);
        }
        
        MAnimLog.Info("[频繁切换测试] 完成 - 所有状态应正确过渡，无权重卡住");
    ...
    void TestStateCache()
    {
        ...
        MAnimLog.Info($"[缓存验证] state1 == state3 ? {cacheWorking}");
        
        if (cacheWorking)
        {
            MAnimLog.Info("[缓存验证] ✅ 成功 - 同一 Clip 返回相同实例");
        }
        else
        {
            MAnimLog.Info("[缓存验证] ❌ 失败 - 缓存机制未生效");
        }
    }
    ...
    void TestNormalizedTime()
    {
        ...
        MAnimLog.Info("[归一化时间] 测试开始");
        MAnimLog.Info($"  动画长度: {state.Length:F2}s");
        
        // 设置归一化时间到 50%
        state.NormalizedTime = 0.5f;
        MAnimLog.Info($"  设置 NormalizedTime = 0.5");
        MAnimLog.Info($"  实际 Time: {state.Time:F2}s");
        MAnimLog.Info($"  实际 NormalizedTime: {state.NormalizedTime:F2}");
        
        MAnimLog.Info("[归一化时间] ✅ API 正常工作");
    }
    ...
    void TestPauseResume()
    {
        ...
        {
            state.Resume();
            MAnimLog.Info("[暂停控制] 已恢复播放");
        }
        else
        {
            state.Pause();
            MAnimLog.Info("[暂停控制] 已暂停播放 - 再按 P 恢复");
        }
    }
    ...
    void TestUpperBodyLayer()
    {
        ...
        if (upperBodyMask != null)
        {
            ...
            MAnimLog.Info("[上半身层] 已设置 AvatarMask");
        }
        else
        {
            MAnimLog.Warning("[上半身层] 未配置 upperBodyMask，请在 Inspector 中设置");
        }
        ...
        if (upperBodyClip != null)
        {
            ...
            MAnimLog.Info("[上半身层] 正在播放上半身动画，下半身保持行走");
        }
        else
        {
            MAnimLog.Warning("[上半身层] 未配置 upperBodyClip，请在 Inspector 中设置");
        }
    }
    ...
    void TestAdditiveLayer()
    {
        ...
        MAnimLog.Info("[叠加层] 已设置为 Additive 模式");

        // 播放叠加动画
        if (breatheClip != null)
        {
            additiveLayer.Play(breatheClip, 0.25f);
            MAnimLog.Info("[叠加层] 正在播放叠加动画（呼吸）");
        }
        else
        {
            MAnimLog.Warning("[叠加层] 未配置 breatheClip，请在 Inspector 中设置");
        }
    }
    ...
    IEnumerator LayerFadeCoroutine()
    {
        ...
        // 淡出层
        MAnimLog.Info("[层淡入淡出] 开始淡出 Layer 1");
        layer1.StartLayerFade(0f, 1f);

        yield return new WaitForSeconds(1.5f);
        MAnimLog.Info($"[层淡入淡出] Layer 1 权重: {layer1.Weight:F2}");

        // 淡入层
        MAnimLog.Info("[层淡入淡出] 开始淡入 Layer 1");
        layer1.StartLayerFade(1f, 1f);

        yield return new WaitForSeconds(1.5f);
        MAnimLog.Info($"[层淡入淡出] Layer 1 权重: {layer1.Weight:F2}");
        MAnimLog.Info("[层淡入淡出] ✅ 测试完成");
    }
    ...
    {
        MAnimLog.Info("[动态创建层] 开始测试");

        // 当前层数
        MAnimLog.Info($"[动态创建层] 当前层数: {animComponent.LayerCount}");

        // 访问 Layer 5，会自动创建 Layer 0~5
        var layer5 = animComponent[5];
        MAnimLog.Info($"[动态创建层] 访问 Layer 5 后，层数: {animComponent.LayerCount}");

        // 验证所有层都已创建
        for (int i = 0; i < animComponent.LayerCount; i++)
        {
            var layer = animComponent[i];
            MAnimLog.Info($"[动态创建层] Layer {i}: {(layer != null ? "已创建" : "null")}");
        }

        MAnimLog.Info("[动态创建层] ✅ 测试完成");
    }
    ...
    {
        MAnimLog.Info("[多层同时播放] 开始测试");

        // Layer 0: 基础层 - 行走
        animComponent.Play(walkClip, 0);
        MAnimLog.Info("[多层同时播放] Layer 0: 行走动画");
        ...
        if (upperBodyClip != null && upperBodyMask != null)
        {
            ...
            MAnimLog.Info("[多层同时播放] Layer 1: 上半身动画（带 Mask）");
        }
        ...
        if (breatheClip != null)
        {
            ...
            MAnimLog.Info("[多层同时播放] Layer 2: 叠加动画（Additive）");
        }

        MAnimLog.Info($"[多层同时播放] 总层数: {animComponent.LayerCount}");
        MAnimLog.Info("[多层同时播放] ✅ 测试完成 - 角色应该同时播放三个动画");
    }
}
}