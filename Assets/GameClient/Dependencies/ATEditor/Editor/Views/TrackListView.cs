...
namespace ATEditor.Editor
{
    ...
    public class TrackListView
    {
        ...
        public TrackListView(ATEditorWindow window, ATEditorState state, ATEditorEvents events)
        {
            this.window = window;
            this.state = state;
            this.events = events;
        ...
        public void DoGUI()
        {
            ...
            float viewportHeight = window.position.height - ATEditorStyles.HEADER_HEIGHT;
            float scrollOffset = state.verticalScrollOffset;

            // 2. 绘制轨道列表内容（带裁剪区域）
            Rect contentGroupRect = new Rect(0, ATEditorStyles.HEADER_HEIGHT, ATEditorStyles.TRACK_LIST_WIDTH, viewportHeight);
            GUI.BeginGroup(contentGroupRect);
            {
                float virtualY = 0; 
                DrawTrackListInGroup(ref virtualY, scrollOffset, viewportHeight, ATEditorStyles.TRACK_LIST_WIDTH);
            ...
            // 3. 处理鼠标事件
            HandleDragAndDrop(new Rect(0, ATEditorStyles.HEADER_HEIGHT, ATEditorStyles.TRACK_LIST_WIDTH, viewportHeight), ATEditorStyles.HEADER_HEIGHT);

            if (evt.type == EventType.MouseDown)
            {
                float totalHeight = state.CalculateTotalHeight() - scrollOffset;
                if (evt.mousePosition.x < ATEditorStyles.TRACK_LIST_WIDTH && evt.mousePosition.y > totalHeight)
                {
                    if (evt.button == 0)
                    {
                        OnLostFocus();
            ...
            // 4. 绘制标题栏
            DrawHeader(ATEditorStyles.TRACK_LIST_WIDTH, ATEditorStyles.HEADER_HEIGHT);
        }
        ...
        private void DrawGroup(Group group, ref float virtualY, float scrollOffset, float viewportHeight, float baseOffset)
        {
            ...
            bool isVisible = (virtualY + ATEditorStyles.GROUP_HEIGHT > scrollOffset && virtualY < scrollOffset + viewportHeight);
            float drawY = virtualY - scrollOffset + baseOffset;
            Rect groupRect = new Rect(0, drawY, ATEditorStyles.TRACK_LIST_WIDTH, ATEditorStyles.GROUP_HEIGHT);
            ...
            if (isVisible)
            {
                ...
                if (GUI.Button(new Rect(4, drawY, 16, ATEditorStyles.GROUP_HEIGHT), arrowIcon, EditorStyles.label))
                {
                    group.isCollapsed = !group.isCollapsed;
                    window.RefreshWindow();
                }
                
                Rect labelRect = new Rect(22, drawY, ATEditorStyles.TRACK_LIST_WIDTH - 26, ATEditorStyles.GROUP_HEIGHT);
                if (renamingGroup == group)
                {
                    ...
                    Rect fieldRect = new Rect(labelRect.x, labelRect.y + (ATEditorStyles.GROUP_HEIGHT - fieldHeight) / 2, labelRect.width, fieldHeight);
            ...
            virtualY += ATEditorStyles.GROUP_HEIGHT;
        ...
        private void DrawTrackItem(TrackBase track, ref float virtualY, float scrollOffset, float viewportHeight, float baseOffset)
        {
            ...
            if (virtualY + ATEditorStyles.TRACK_HEIGHT < scrollOffset || virtualY > scrollOffset + viewportHeight)
            {
                virtualY += ATEditorStyles.TRACK_HEIGHT;
                return;
            }

            float drawY = virtualY - scrollOffset + baseOffset;
            Rect trackRect = new Rect(0, drawY, ATEditorStyles.TRACK_LIST_WIDTH, ATEditorStyles.TRACK_HEIGHT);
            ...
            Rect textRect = new Rect(32, drawY, ATEditorStyles.TRACK_LIST_WIDTH - 36, ATEditorStyles.TRACK_HEIGHT);
            if (renamingTrack == track)
            {
                ...
                float fieldHeight = 18f;
                Rect fieldRect = new Rect(textRect.x, textRect.y + (ATEditorStyles.TRACK_HEIGHT - fieldHeight) / 2, textRect.width, fieldHeight);
                renamingText = EditorGUI.TextField(fieldRect, renamingText);
            }
            else
            {
            ...
            EditorGUI.DrawRect(new Rect(0, drawY + ATEditorStyles.TRACK_HEIGHT - 1, ATEditorStyles.TRACK_LIST_WIDTH, 1), new Color(0.1f, 0.1f, 0.1f));
            virtualY += ATEditorStyles.TRACK_HEIGHT;
        }
        ...
        private void CreateNewGroup()
        {
            ...
            ATLog.Info($"创建分组: {newGroup.groupName}");
            events.OnRepaintRequest?.Invoke();
        }
        ...
        private void OnAddTrackToGroup(Group group, Type trackType)
        {
            ...
            ATLog.Info($"成功添加轨道到分组: {newTrack.trackName} -> {group.groupName}");
            events.OnRepaintRequest?.Invoke();
        }
        ...
        private TrackBase CreateTrack(Type trackType)
        {
            ...
            {
                ATLog.Error($"创建轨道失败: {trackType.Name}, 错误: {e.Message}");
                return null;
            }
        }
        ...
        private void DeleteTrack(TrackBase track, bool showDialog = false)
        {
            ...
            ATLog.Info($"已删除轨道: {track.trackName}");
            events.OnRepaintRequest?.Invoke();
        }
        ...
        private void DeleteGroup(Group group, bool showDialog = true)
        {
            ...
            ATLog.Info($"已删除分组: {group.groupName}");
            events.OnRepaintRequest?.Invoke();
        }
        ...
        private void UpdateDropTarget(float virtualMouseY)
        {
            ...
            if (timeline.Groups != null)
            {
                for (int i = 0; i < timeline.Groups.Count; i++)
                {
                    ...
                    float groupTop = y;
                    float groupBottom = y + ATEditorStyles.GROUP_HEIGHT;
                    ...
                    if (virtualMouseY >= groupTop && virtualMouseY <= groupBottom)
                    {
                        ...
                        {
                            dropTargetIndex = i;
                            isDropAfter = virtualMouseY > (groupTop + ATEditorStyles.GROUP_HEIGHT / 2);
                            dropIndicatorY = isDropAfter ? groupBottom : groupTop;
                        }
                        return;
                    }
                    y += ATEditorStyles.GROUP_HEIGHT;
                    ...
                    if (!group.isCollapsed && group.tracks != null)
                    {
                        ...
                        {
                            float trackTop = y;
                            float trackBottom = y + ATEditorStyles.TRACK_HEIGHT;
                            if (virtualMouseY >= trackTop && virtualMouseY <= trackBottom)
                            {
                                if (isDraggingTrack)
                                {
                                    ...
                                    isDropAfter = virtualMouseY > (trackTop + ATEditorStyles.TRACK_HEIGHT / 2);
                                    dropIndicatorY = isDropAfter ? trackBottom : trackTop;
                                }
                                return;
                            }
                            y += ATEditorStyles.TRACK_HEIGHT;
                        }
                    }
                }
            }
        ...
        private void CopyTrack(TrackBase track)
        {
            ...
            ATLog.Info($"已复制轨道: {track.trackName}");
        }
        ...
        private void PasteTrack(Group targetGroup)
        {
            ...
            ATLog.Info($"已粘贴轨道: {newTrack.trackName} -> {targetGroup.groupName}");
            events.OnRepaintRequest?.Invoke();
        }
        ...
        private void CopyGroup(Group group)
        {
            ...
            ATLog.Info($"已复制分组: {group.groupName} (包含 {state.copiedTracksForGroup.Count} 条轨道)");
        }
        ...
        private void PasteGroup()
        {
            ...
            ATLog.Info($"已粘贴分组: {newGroup.groupName} (包含 {newGroup.tracks.Count} 条轨道)");
            events.OnRepaintRequest?.Invoke();
        }
        ...
        private void SelectTrack(TrackBase track)
        {
            ...
            ATLog.Info($"选中轨道: {track.trackName}");
        }
        ...
        private void SelectGroup(Group group)
        {
            ...
            ATLog.Info($"选中分组: {group.groupName}");
        }
        ...
        private void DuplicateGroup(Group sourceGroup)
        {
            ...
            ATLog.Info($"已完成分组克隆: {newGroup.groupName}");
            events.OnRepaintRequest?.Invoke();
        }
        ...
        private void DuplicateTrack(TrackBase sourceTrack)
        {
            ...
            ATLog.Info($"已完成轨道克隆: {newTrack.trackName}");
            events.OnRepaintRequest?.Invoke();
        }
