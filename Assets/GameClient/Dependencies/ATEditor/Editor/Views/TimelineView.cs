...
namespace ATEditor.Editor
{
    ...
    public class TimelineView
    {
        ...
        public TimelineView(ATEditorWindow window, ATEditorState state, ATEditorEvents events)
        {
            this.window = window;
            this.state = state;
            this.events = events;
        ...
        public void DoGUI(Rect rect)
        {
            ...
            Rect tracksGroupRect = new Rect(0, ATEditorStyles.TIME_RULER_HEIGHT, rect.width, viewHeight - ATEditorStyles.TIME_RULER_HEIGHT);
            GUI.BeginGroup(tracksGroupRect);
            {
                DrawTracksArea(new Rect(0, -ATEditorStyles.TIME_RULER_HEIGHT, rect.width, viewHeight));
            }
            GUI.EndGroup();
        ...
        private void DrawTracksArea(Rect rect)
        {
            ...
            if (timeline.Groups != null)
            {
                ...
                for (int i = 0; i < timeline.Groups.Count; i++)
                {
                    ...
                    bool isGroupVisible = (yOffset + ATEditorStyles.GROUP_HEIGHT > scrollOffset && yOffset < scrollOffset + viewportHeight);
                    if (isGroupVisible)
                    {
                        float drawY = yOffset - scrollOffset;
                        Rect groupRect = new Rect(0, drawY, rect.width, ATEditorStyles.GROUP_HEIGHT);
                        ...
                        EditorGUI.DrawRect(new Rect(groupRect.x, groupRect.y + ATEditorStyles.GROUP_HEIGHT - 1, groupRect.width, 1), new Color(0.1f, 0.1f, 0.1f));
                    }

                    yOffset += ATEditorStyles.GROUP_HEIGHT;
                    ...
                    if (!group.isCollapsed && group.tracks != null)
                    {
                        for (int j = 0; j < group.tracks.Count; j++)
                        {
                            ...
                            if (track != null)
                            {
                                ...
                                float trackHeight = ATEditorStyles.TRACK_HEIGHT;
        ...
        private void DrawClipsOnTrack(TrackBase track, Rect trackRect)
        {
            ...
            for (int i = 0; i < track.clips.Count; i++)
            {
                ...
                Rect clipRect = new Rect(trackRect.x + clipStartX, trackRect.y + ATEditorStyles.CLIP_MARGIN_TOP, clipWidth, ATEditorStyles.CLIP_HEIGHT);
                
                // 获取 Drawer 绘制所需的预计算变量
                Color clipColor = clipInteraction.GetClipColor(track.trackType);

                // 计算 Drawer 所需的文本截断显示名称
        ...
        private void DrawSnapLine(Rect rect)
        {
            ...
            for (float y = ATEditorStyles.TIME_RULER_HEIGHT; y < rect.height; y += 10)
            {
        ...
        private void HandlePanning(Rect rect)
        {
            ...
            if (e.type == EventType.MouseDown && (e.button == 1 || e.button == 2) && e.mousePosition.y >= ATEditorStyles.TIME_RULER_HEIGHT)
            {
                isPanning = true;
        ...
        private void ProcessContextClick(Vector2 mousePosition)
        {
            ...
            float vMouseY = mousePosition.y - ATEditorStyles.TIME_RULER_HEIGHT + vScroll;
            ...
            if (timeline.Groups != null)
            {
                ...
                {
                    var group = timeline.Groups[i];
                    currentVirtualY += ATEditorStyles.GROUP_HEIGHT;
                    ...
                    if (!group.isCollapsed && group.tracks != null)
                    {
                        for (int j = 0; j < group.tracks.Count; j++)
                        {
                            ...
                            if (track != null)
                            {
                                if (vMouseY >= currentVirtualY && vMouseY < currentVirtualY + ATEditorStyles.TRACK_HEIGHT)
                                {
                                    ...
                                    if (track.clips != null)
                                    {
                                        for (int k = 0; k < track.clips.Count; k++)
                                        {
                                            ...
                                            float clipStartX = coords.TimeToPhysX(clip.StartTime);
                                            float clipWidth = clip.Duration * state.zoom;
                                            float drawY = currentVirtualY - vScroll + ATEditorStyles.TIME_RULER_HEIGHT;
                                ...
                                currentVirtualY += ATEditorStyles.TRACK_HEIGHT;
                            }
                        }
                    }
                }
        ...
        private void HandleMouseEvents(Rect rect)
        {
            ...
            if (e.type == EventType.MouseDown && e.mousePosition.y >= ATEditorStyles.TIME_RULER_HEIGHT)
            {
                ...
                float vScroll = state.verticalScrollOffset;
                float vMouseY = e.mousePosition.y - ATEditorStyles.TIME_RULER_HEIGHT + vScroll;
                float currentVirtualY = 0;
            ...
            if (e.type == EventType.MouseUp && isBoxSelecting)
            {
                ...
                if (timeline != null && isRealDrag)
                {
                    ...
                    float yOffset = ATEditorStyles.TIME_RULER_HEIGHT;

                    System.Action<TrackBase> checkBoxTrack = (track) => {
                        Rect trackRect = new Rect(0, yOffset, rect.width, ATEditorStyles.TRACK_HEIGHT);
                        foreach (var clip in track.clips)
                        {
                            float clipStartX = coords.TimeToPhysX(clip.StartTime);
                            float clipWidth = clip.Duration * state.zoom;
                        ...
                        yOffset += ATEditorStyles.TRACK_HEIGHT;
                    };
                    ...
                    ATLog.Info($"框选结束，选中了 {state.selectedClips.Count} 个片段");
                }
            ...
            if (e.type == EventType.KeyDown && e.keyCode == KeyCode.A && (e.control || e.command))
            {
                ...
                if (timeline != null)
                {
                    ...
                    ATLog.Info($"全选了 {state.selectedClips.Count} 个片段");
                    events.NotifySelectionChanged();
                }
                e.Use();
            }
            ...
            if (e.type == EventType.ScrollWheel && (e.control || e.command))
            {
                ...
                float newZoom = Mathf.Clamp(state.zoom + zoomDelta, ATEditorStyles.MIN_ZOOM, ATEditorStyles.MAX_ZOOM);
                state.zoom = newZoom;
