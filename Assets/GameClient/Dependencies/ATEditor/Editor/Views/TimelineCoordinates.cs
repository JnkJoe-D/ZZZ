...
namespace ATEditor.Editor
{
    /// <summary>
    /// 时间轴坐标转换、吸附和刻度计算工具籀
    /// 无状态辅助类，通过 ATEditorState 读取缩放/偏移等参敀
    /// </summary>
    public class TimelineCoordinates
    {
        ...
        // 常量
        public const float TIMELINE_START_OFFSET = 10f; // 时间轴左侧偏移量（防歀0 刻度遮挡＀

        public TimelineCoordinates(ATEditorState state)
        {
            this.state = state;
        }
        ...
        /// <param name="snapped">是否吸附到某个目栀/param>
        /// <param name="minPixelDist">最小像素距禀/param>
        ...
        public void CalculateRulerLevels(out float majorStep, out float subStep, out float gridStep, out bool useFrameIndex)
        {
            ...
            // 2. 如果 Grid 本身就很宀(>45px)，则直接作为 Major
        ...
        public void AutoResolveBlending(TrackBase track, ClipBase modifiedClip)
        {
            ...
            foreach (var clip in track.clips)
            {
                ...
                // Case 1: modifiedClip 头部 overlap 亀clip 的尾郀
                ...
                // Case 2: modifiedClip 尾部 overlap 亀clip 的头郀
