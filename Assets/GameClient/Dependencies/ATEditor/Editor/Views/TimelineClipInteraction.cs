...
namespace ATEditor.Editor
{
    ...
    public class TimelineClipInteraction
    {
        ...
        public TimelineClipInteraction(ATEditorWindow window, ATEditorState state, ATEditorEvents events,
                    ...
                    if (hoveredTrack != null && dragSourceTrack != null && hoveredTrack != dragSourceTrack)
                    {
                        ...
                        if (allCompatible)
                        {
                            if (CurrentDragMode != DragMode.CrossTrackDrag)
                            {
                                ...
                                ATLog.Info($"进入多选跨轨道拖拽模式: {hoveredTrack.trackName}");
                            }
                        }
                        else
                        {
            ...
            if (e.type == EventType.MouseUp && e.button == 0 && CurrentDragMode != DragMode.None)
            {
                if (CurrentDragMode == DragMode.CrossTrackDrag)
                {
                    ...
                    if (DragHoveredTrack != null)
                    {
                        ...
                        ATLog.Info($"多选片段移动完成，主片段迁移至: {DragHoveredTrack.trackName}");
                    }
                    else
                    {
                        ATLog.Warning("跨轨道移动取消：目标位置无效。");
                    }
