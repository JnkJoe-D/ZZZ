...
namespace ATEditor.Editor
{
    ...
    public class AnimationTrackDrawer : TrackDrawer
    {
        ...
        private static ActionTimeline GetCurrentTimeline()
        {
            ...
            // 兜底：从 ATEditorWindow 获取
            var windows = Resources.FindObjectsOfTypeAll<ATEditorWindow>();
            if (windows.Length > 0)
            {
                // ATEditorWindow 有 state.currentTimeline
                var stateField = typeof(ATEditorWindow).GetField("state",
                    BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance);
                if (stateField != null)
                {
                    var state = stateField.GetValue(windows[0]) as ATEditorState;
