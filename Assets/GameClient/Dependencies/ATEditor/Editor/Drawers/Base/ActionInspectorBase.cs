...
namespace ATEditor.Editor
{
    ...
    /// 基于反射的 Inspector 基类
    /// </summary>
    public class ActionInspectorBase
    {
        ...
        protected void DrawDefaultInspector(object obj)
        {
            ...
            foreach (var type in typeHierarchy)
            {
                ...
                {
                    if (!field.IsPublic && !field.IsDefined(typeof(ActionPropertyAttribute), true))
                    {
                        continue;
                    }
                ...
                {
                    if (prop.IsDefined(typeof(ActionPropertyAttribute), true))
                    {
        ...
        protected virtual bool ShouldShow(FieldInfo field, object obj)
        {
            ...
            var showIf = field.GetCustomAttribute<ATShowIfAttribute>();
            if (showIf != null)
            {
        ...
        protected virtual void DrawField(FieldInfo field, object obj)
        {
            ...
            var attribute = field.GetCustomAttribute<ActionPropertyAttribute>();
            ...
            {
                // Unity 内置用于 EditorGUILayout 的 LayerMask 扩展
            ...
            else if (value is ActionAssetReference assetRef)
            {
                var attr = field.GetCustomAttribute<ActionAssetReferenceAttribute>();
                if (attr != null)
                {
            ...
            else if (value is List<ActionAssetReference> assetRefList)
            {
                var attr = field.GetCustomAttribute<ActionAssetReferenceAttribute>();
                if (attr != null)
                {
                    ...
                    if (targetField != null)
                    {
                        ...
                        if (targetList != null)
                        {
                            while (assetRefList.Count < targetList.Count) assetRefList.Add(new ActionAssetReference());
        ...
        private void UpdateAssetReference(ActionAssetReference assetRef, Object target)
        {
            if (target == null)
            {
        ...
        private static void LoadHitEffectData()
        {
            ...
            {
                ATLog.Error($"加载 HitEffect JSON 失败: {ex.Message}");
            }
        }
        ...
        public static void LoadBuffData()
        {
            ...
            {
                ATLog.Error($"加载 Buff JSON 失败: {ex.Message}");
            }
        }
    }
}
