    ...
    public class EditorHitProcess : ProcessBase<HitClip>
    {
        ...
        public override void OnEnter()
        {
            if (clip.detectFrequency == Frequency.Once)
            {
                ...
                ATLog.Info($"<color=orange>Damage Triggered!</color> HitEffectId: {detect?.hitEffectId ?? 0}, Time: OnEnter");
            }
            lastCheckTime = -1f;
            timesChecked = 0;
        ...
        public override void OnUpdate(float currentTime, float deltaTime)
        {
            if (clip.detectFrequency == Frequency.Times)
            {
                ...
                {
                    var detect = clip.SelectedDetect;
                    ATLog.Info($"<color=orange>Damage Triggered (Times)!</color> HitEffectId: {detect?.hitEffectId ?? 0}, Time: {currentTime:F2}, Checks: {timesChecked + 1}/{clip.times}");
        ...
        public void GetMatrix(out Vector3 pos, out Quaternion rot)
        {
            ...
            if (context != null && context.Owner != null)
            {
                ...
                bindTrans = boneGetter.GetBone(clip.bindPoint, clip.customBoneName);
            }
