...
{
    /// <summary>
    /// 仅供编辑器的动画预览运行时，基于 Unity Playables 实现。
    /// 该运行时绕过 AnimComponent/ISkillAnimationHandler，直接对动作切片进行时间轴求值采样。
    /// </summary>
    public static class EditorAnimationUtils
    {
        ...
        private static void RebuildIfDirty(PreviewGraphContext ctx)
        {
            ...
            {
                AnimationMixerPlayable layerMixer = AnimationMixerPlayable.Create(ctx.graph, 0);
