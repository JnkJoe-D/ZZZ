...
namespace ATEditor
{
    ...
    public static class SerializationUtility
    {
        ...
        public static void SaveDual(ActionTimeline timeline, string jsonDir, string assetDir, string fileName)
        {
            ...
            ATLog.Info($"[ActionTimeline] 双轨保存完成:\nSO: {assetPath}\nJSON: {jsonPath}");
        }
        ...
        private static async Task<T> ResolveAsset<T>(string guid, string assetPath, string assetName = null) where T:Object
        {
            ...
            if (Application.isPlaying && Runtime.ActionSystemContext.AssetLoader != null)
            {
                ...
                {
                    T subAsset = await Runtime.ActionSystemContext.AssetLoader.LoadSubAssetAsync<T>(address, assetName);
                    if (subAsset != null) return subAsset;
                }
                
                // 否则直接加载主资产
                T runtimeAsset = await Runtime.ActionSystemContext.AssetLoader.LoadAssetAsync<T>(address);
        ...
        private static T ResolveAssetImmediate<T>(string guid, string assetPath, string assetName = null) where T : Object
        {
            ...
            if (Application.isPlaying && Runtime.ActionSystemContext.AssetLoader != null)
            {
                ...
                if (!string.IsNullOrEmpty(assetPath) && !string.IsNullOrEmpty(assetName) &&
                    (typeof(T) == typeof(UnityEngine.AnimationClip) || typeof(T) == typeof(AvatarMask)))
                {
                    T subAsset = Runtime.ActionSystemContext.AssetLoader.LoadSubAsset<T>(address, assetName);
                    if (subAsset != null) return subAsset;
                }

                T runtimeAsset = Runtime.ActionSystemContext.AssetLoader.LoadAsset<T>(guid, address);
                if (runtimeAsset != null) return runtimeAsset;
            }
