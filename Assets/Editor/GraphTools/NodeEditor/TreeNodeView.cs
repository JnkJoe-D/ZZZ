...
namespace Game.Editor.BehaviorTree
{
    public class TreeNodeView : UnityEditor.Experimental.GraphView.Node
    {
        ...
        public TreeNodeView(NodeData node)
        {
            this.node = node;
            title = node.name;
        ...
        private void SetupPortLayout()
        {
            ...
            if (node != null)
            {
                ...
                var attributes = type.GetCustomAttributes(typeof(NodeColorAttribute), false);
                if (attributes.Length > 0)
                {
                    var colorAttr = attributes[0] as NodeColorAttribute;
        ...
        public void CreateOutputPort()
        {
            ...
            var capacity = (node is DecoratorData||
            node is RootData) ? Port.Capacity.Single : Port.Capacity.Multi;
        ...
        public void UpdateState(BTRunner runner)
        {
