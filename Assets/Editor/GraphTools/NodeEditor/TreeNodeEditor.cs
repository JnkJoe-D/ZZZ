...
namespace Game.Editor.BehaviorTree
{
    public class TreeNodeEditor : EditorWindow
    {
        ...
        public BTRunner currentRunner;
        public InspectorViewr inspectorView;

        private bool _isLoadedFromDisk;
        private int _initialNodeCountOnLoad;
        private bool _hasDiskAssetBeenModified;
        ...
        public void CreateGUI()
        {
            ...
            {
                m_VisualTreeAsset = AssetDatabase.LoadAssetAtPath<VisualTreeAsset>("Assets/Editor/GraphTools/NodeEditor/TreeNodeEditor.uxml");
            }
            if (m_VisualTreeAsset != null)
            {
            ...
            {
                Debug.LogError("[TreeNodeEditor] Failed to load VisualTreeAsset at 'Assets/Editor/GraphTools/NodeEditor/TreeNodeEditor.uxml'");
                return;
            }
            ...
            {
                inspectorView.onNodeModified = OnGraphModified;
            }
        ...
        public void LoadTree(BehaviorTreeAsset tree)
        {
            ...
            if (tree == null)
            {
                ...
                _initialNodeCountOnLoad = 0;
                _hasDiskAssetBeenModified = false;
            }
            else
            {
                string assetPath = AssetDatabase.GetAssetPath(tree);
                _isLoadedFromDisk = !string.IsNullOrEmpty(assetPath);
                _initialNodeCountOnLoad = tree.nodes != null ? tree.nodes.Count : 0;
                _hasDiskAssetBeenModified = false;
            }
        ...
        {
            _hasDiskAssetBeenModified = true;
            UpdateUnsavedChangesState();
        }
        ...
        private void UpdateUnsavedChangesState()
        {
            ...
            var tree = graphView.tree;
            int currentNodeCount = tree.nodes != null ? tree.nodes.Count : 0;
            bool isSavedOnDisk = !string.IsNullOrEmpty(AssetDatabase.GetAssetPath(tree));

            // 1. 打开窗口时默认创建的全新临时资产（未保存到磁盘）
            if (!isSavedOnDisk && !_isLoadedFromDisk)
            {
                ...
                else
                {
                    ...
                    saveChangesMessage = "当前新建行为树尚未保存，关闭将丢失所有节点数据。是否保存到磁盘？";
                }
                return;
            }

            // 2. 打开已有的有效树资产（来自磁盘或已落盘）
            if (isSavedOnDisk || _isLoadedFromDisk)
            {
                // 如果用户把所有结点删除了（从原本有节点变为 0 个，或清空了已有资产）
                if (currentNodeCount == 0)
                {
                    if (_initialNodeCountOnLoad > 0 || _hasDiskAssetBeenModified)
                    {
                        // 删除了原有所有节点：必须提示
                        hasUnsavedChanges = true;
                        saveChangesMessage = $"行为树资产 '{tree.name}' 的所有节点已被清空，是否保存更改？";
                    }
                    else
                    {
                        hasUnsavedChanges = false;
                ...
                {
                    // 结点数 > 0，如果图或节点被修改过则提示未保存
                    hasUnsavedChanges = _hasDiskAssetBeenModified;
                    saveChangesMessage = $"行为树资产 '{tree.name}' 已被修改，是否保存更改到磁盘？";
                }
            }
        }
        ...
        {
            if (hasUnsavedChanges && graphView != null && graphView.tree != null)
            {
                ...
                else if (graphView.tree.nodes != null && graphView.tree.nodes.Count > 0)
                {
                    SaveAsTree();
                }
            }
